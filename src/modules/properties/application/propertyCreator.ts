...
export default class PropertyCreator {
  ...
  async run(req: CreatePropertyRequest): Promise<void> {
    const existingByCode = await this.repository.findByCodigoCatastral(new PropertyCadastralCode(req.cadastralCode));
    if (existingByCode) {
    ...
    const property = new Property(
      ...
      new PropertyCadastralCode(req.cadastralCode),
